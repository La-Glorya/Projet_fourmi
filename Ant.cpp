...
Ant::Ant(std::vector<std::vector<Element>>& grid, float _x, float _y, int _couleur) : state(AntState::FOLLOW), pos(Vector(_x, _y)), x(_x), y(_y), couleur(_couleur), carryFood(false), grid(grid), historyIndex(0), posHistory(new Vector[1000])
{}
...
void Ant::update(sf::RenderTexture& tex)
{
	...
	if (state == AntState::FOLLOW)
		...
		processReturn();
    if(couleur == 2)
        couleurs = FOURM2_COLOR;
    if(couleur == 3)
        couleurs = FOURM3_COLOR;
	int color = state == AntState::FOLLOW || !carryFood ? couleurs : ANT_COLOR_RETURN;
